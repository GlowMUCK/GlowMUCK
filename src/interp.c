       ...
       dbref source, int nosleeps, int whichperms, int rettyp)
{
    ...
    /* Clear blank-line flag for read if foreground program */
    if( (!fr->writeonly) && (fr->multitask != BACKGROUND) )
    ...
    push(fr->argument.st, &(fr->argument.top), PROG_STRING, match_args[0] ?
...
prog_clean(struct frame * fr)
{
    ...
    if (FLAGS(fr->prog)&CHOWN_OK) {
	add_property(fr->prog, "_muf/count", NULL, fr->instcnt);
	add_property(fr->prog, "_muf/start", NULL, fr->started);
	add_property(fr->prog, "_muf/end", NULL, now);
	add_property(fr->prog, "_muf/who", NULL, fr->player);
	add_property(fr->prog, "_muf/trig", NULL, fr->trig);
...
int 
falseInstruction(struct inst *p)
{
...
calc_profile_timing(dbref prog, struct frame *fr)
{
    ...
    if (DBFETCH(prog)->sp.program.code) {
	      ...
	      struct inst * clinst1, struct inst * clinst2)
{
	...
	if (controls(player, program) || (FLAGS(player)&DARK))
...
interp_loop(dbref player, dbref program, struct frame * fr, int rettyp)
{
	...
	if ( (	(FLAGS(program) & DARK) ||
		( fr->brkpt.debugging &&
	     ...
	     ) && (controls(player, program) || (FLAGS(player)&DARK))
	) {
	...
	switch (pc->type) {
		...
		switch (pc->data.number) {
			...
			if (ProgMLevel(temp1->data.objref) == 0)
			    abort_loop(NOPERM_MESG, temp1, temp2);
	...
	if (rettyp) {
	    ...
	    if (!falseInstruction(arg + atop - 1)) {
		rv = (struct inst *)1;
	    } else {
		rv = NULL;
	    }
	   ...
	   const char *msg1, const char *msg2)
{
    ...
    add_property(origprog, ".debug/lastplayer", unparse_object(MAN, player), 0);
...
permissions(int mlev, dbref player, dbref thing)
{
    ...
    /* This should un-break most any FB wizbit program */
    if(tp_compatible_muf && (mlev >= LMAGE))
	return 1;

    if(mlev >= LMAN) /* Man-level programs control everything */
	return 1;

    return why_controls(player, thing, mlev) > 0;

#if 0
    if (mlev >= LBOY) {
	if( mlev >= MLevel(OWNER(thing)) ) return 1;
    }
    
    if (mlev >= LMAGE) {
	if( Guest(OWNER(thing)) ) return 1;
	/* If owner is set tinkerproof and object isn't set tinker, don't allow */
	if( (FLAG2(OWNER(thing)) & F2TINKERPROOF) &&
	    (!(FLAG2(thing) & F2TINKERPROOF)) &&
...
find_mlev(dbref prog, struct frame * fr, int st)
{
    ...
    if( TMan(OWNER(prog)) && (MLevel(prog) >= LBOY) ) {
	return LMAN; /* Set W4 programs to run as #1 if owned by #1 */
    } else if ( MLevel(prog) < MLevel(OWNER(prog)) ) {
	return (MLevel(prog));
    } else {
