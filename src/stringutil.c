...
pronoun_substitute(dbref player, const char *str)
{
    ...
    static const char *subjective[5] = {"", "it", "she", "he", "shi"};
...
/* NOTE: string must be of size BUFFER_LEN, no checking is done for this */
void 
string_subst(char *string, const char *match, const char *replacement)
{
    int i = 0, j = 0, k, l;
    char buf[BUFFER_LEN];

    if((!*string) || (!match) || (!*match) || (!replacement) || (!*replacement))
	return; /* Bad strings or nothing to do */
    ...
    while (string[i]) {
	if (!strncmp(string + i, match, l)) {
	    if ((j + k + 1) > BUFFER_LEN)
		break; /* End of the line, outta space */
    ...
    /* Copy resultant string into 'string', make sure string is BUFFER_LEN */
    strcpy(string, buf);
}
...
strencrypt(const char *data, const char *key)
{
	...
	seed2 = ((seed2 + 1) & 0x3f);
	if (!*(++cp)) cp = key;
	result = (enarr[(signed char) *upt] - (32 - (CHARCOUNT - 96))) + count + seed;
...
strdecrypt(const char *data, const char *key)
{
	...
	result = (enarr[(signed char)*upt] - (32 - (chrcnt - 96))) - (count + seed);
