...
struct match_data {
    ...
    int allow_muf_verbs;	/* block matching of muf verbs with spaces */
};
...
extern void init_match_check_keys(dbref player, const char *name, int type,
				  struct match_data *md);
extern void init_match_remote(dbref player, dbref what, const char *name,
			int type, struct match_data * md);
extern void set_match_muf_verbs(int allow, struct match_data * md);
...
extern const char *match_detail( dbref thing, const char *detail );

#define MatchOk(x) ( ((x) != (PropPtr)0L) && ((x) != (PropPtr)NOTHING) && ((x) != (PropPtr)AMBIGUOUS) && ((x) != (PropPtr)HOME) )
