...
mfn_listprops(MFUNARGS)
{
    ...
    *buf = '\0';
	...
	if (ptr && *ptr) {
	    flag = 1;
	    if(Permlevel(what) < tp_hidden_prop_mlevel) {
		if (Prop_Hidden(ptr)) {
		    flag = 0;
		}
	    }
	    if(Permlevel(what) < tp_private_prop_mlevel) {
...
mfn_timesub(MFUNARGS)
{
    ...
    offset = ((((time_t)current_systime + offset) % period) * num) / period;
...
mfn_time(MFUNARGS)
{
	...
	tm = localtime(&lt);
#endif
    format_time(buf, BUFFER_LEN - 1, "%T\0", tm);
    return buf;
}
...
mfn_ltimestr(MFUNARGS)
{
    ...
    *buf = '\0';
    ...
    if (dy) {
	if (*buf) {
	    sprintf(buf + strlen(buf), ", %d day%s", dy, (dy == 1)? "" : "s");
	} else {
    ...
    if (hr) {
	if (*buf) {
	    sprintf(buf + strlen(buf), ", %d hour%s", hr, (hr == 1)? "" : "s");
	} else {
    ...
    if (mn) {
	if (*buf) {
	    sprintf(buf + strlen(buf), ", %d min%s", mn, (mn == 1)? "" : "s");
	} else {
    ...
    if (tm || !*buf) {
	if (*buf) {
	    sprintf(buf + strlen(buf), ", %d sec%s", tm, (tm == 1)? "" : "s");
	} else {
...
mfn_nearby(MFUNARGS)
{
    ...
    if (!Mageperms(what) && !isneighbor(obj, what) && !isneighbor(obj2, what) &&
...
mfn_oansi(MFUNARGS)
{
	...
	while (thing != NOTHING) {
	    if (thing != eobj)
		ansi_notify_nolisten(thing, buf, 0, 1);
