...
void 
prim_ansi_offset(PRIM_PROTOTYPE)
{
    CHECKOP(3);
    oper1 = POP();
    oper2 = POP();
	...
	/* Cooked text position -> raw offset */
	result = ansi_offset(oper3->data.string->data, tmp, result);
    } else result = 0;
    CLEAR(oper1);
    CLEAR(oper2);
    CLEAR(oper3);
...
prim_ansi_strip(PRIM_PROTOTYPE)
{
    ...
    buf[0] = '\0';
    if (oper1->data.string) {
	unparse_ansi(buf, oper1->data.string->data, 1);
    }
    CLEAR(oper1);
    PushString(buf);
}
...
prim_ansi_notify(PRIM_PROTOTYPE)
{
	...
	ansi_notify_listeners(player, program, oper2->data.objref,
			      getloc(oper2->data.objref), buf, 1, 1);
    }
    CLEAR(oper1);
    CLEAR(oper2);
}
...
prim_ansi_notify_exclude(PRIM_PROTOTYPE)
{
	...
	if (*buf) {
		...
		if (!tmp)
		    ansi_notify_listeners(player, program, what, where, buf, 0, 1);
		what = DBFETCH(what)->next;
	    }
	}

	if (tp_listeners) {
	    ansi_notify_listeners(player, program, where, where, buf, 0, 1);
	    if (tp_listeners_env) {
