...
RETSIGTYPE sig_restart(int);
RETSIGTYPE sig_dump(int);
...
#if !defined(SYSV) && !defined(_POSIX_VERSION) && !defined(ULTRIX) && !defined(WIN95)
    return 0;
#endif
}
...
RETSIGTYPE sig_reap_resolver(int i)
{
	...
	/* Pakfix for getting the resolver to stay after doing a system() */
	/* kill_resolver(); */
#endif
