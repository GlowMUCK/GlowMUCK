...
void 
#ifdef MUD
putfref(FILE * f, dbref ref, dbref ref2, int hit, int mgc, int exp, int wgt, dbref ldr)
{
    if (fprintf(f, "%d %d %d %d %d %d %d\n",
	ref, ref2, hit, mgc, exp, wgt, ldr) == EOF
    ) {
	abort();
#else
...
file_line(FILE * f)
{
	...
	return NULL;
    len = 0;
    while((buf[len] >= ' ') && (len < (BUFFER_LEN - 1)))
	len++;
    buf[len] = '\0';
...
# endif
	...
	HIT(i), MGC(i), EXP(i), WGT(i), getleader(i)
    );
#else
    putfref(f, (FLAGS(i) & ~DUMP_MASK), (FLAG2(i) & ~DUM2_MASK));
#endif
...
dbref 
#ifdef MUD
getfref(FILE * f, dbref *f2, int *hit, int *mgc, int *exp, int *wgt, dbref *ldr)
{
    char buf[BUFFER_LEN];
    dbref f1;
    int got, peekch;
    ...
    got = sscanf(buf, "%d %d %d %d %d %d %d", &f1, f2, hit, mgc, exp, wgt, ldr);
...
read_program(dbref i)
{
    ...
    while (fgets(buf, BUFFER_LEN / 2, f)) {
	if(tp_purify_muf_files) {
	    c = 0;
	    while(buf[c]) {
		if((buf[c] == '\n') || (buf[c] == '\r'))
		    break;
		/* As far as I can tell '~' is the highest muf character. */
		if((buf[c] < ' ') || (buf[c] > '~'))
		    buf[c] = ' ';
		c++;
	    }
	...
	while(buf[len] && (buf[len] != '\n') &&
	    (buf[len] != '\r') && (len < (BUFFER_LEN / 2))
	) len++;
	buf[len] = '\0';
	/* Well, blank lines would crash before, so why keep em now? */
...
db_read_object_old(FILE * f, struct object * o, dbref objno)
{
...
#ifdef MUD
    FLAGS(objno) |= getfref(f, &f2, &hit, &mgc, &exp, &wgt, &ldr);
    HIT(objno)=hit;
    MGC(objno)=mgc;
    EXP(objno)=exp;
    WGT(objno)=wgt;
	...
	case GENDER_NEUTER:
	    add_property(objno, PROP_SEX, "neuter", 0);
	    break;
	case GENDER_FEMALE:
	    add_property(objno, PROP_SEX, "female", 0);
	    break;
	case GENDER_MALE:
	    add_property(objno, PROP_SEX, "male", 0);
	    break;
	default:
	    break;
    }
...
#ifdef MUD
    tmp = getfref(f, &f2, &hit, &mgc, &exp, &wgt, &ldr);
    HIT(objno)=hit;
    MGC(objno)=mgc;
    EXP(objno)=exp;
    WGT(objno)=wgt;
...
db_read(FILE * f)
{
	...
	if (!strcmp(special, "**TinyMUCK DUMP Format***")) {
	    ...
	    (!strcmp(special, "**Foxen5 TinyMUCK DUMP Format***")) ||
...
int
RawMLevel(dbref player)
{
    if(!OkObj(player))
	return 0;

    return tp_multi_wiz_levels
    ?	( ( ((FLAGS(player) & W3) ? 1 : 0) << 2 ) +	\
	  ( ((FLAGS(player) & W2) ? 1 : 0) << 1 ) +	\
	  ( ((FLAGS(player) & W1) ? 1 : 0)      )	)
    :	( ( ((FLAGS(player) & W2) ? 1 : 0) << 1 ) +	\
	  ( ((FLAGS(player) & W1) ? 1 : 0)      ) 	)
    ;
...
MLevel(dbref player)
{
    ...
    return tp_multi_wiz_levels
    ?	( TMan(player) ? LMAN : RawMLevel(player) )
    :	( TMan(player) ? LMAN : \
	  ((FLAGS(player) & W3) ? LARCH : RawMLevel(player) ) \
	)
    ;
}
...
void SetMLevel(dbref player, int mlev)
{
    if(!OkObj(player))
	return;

    FLAGS(player) = tp_multi_wiz_levels
    ?	( FLAGS(player) & ~(W1|W2|W3) ) | \
	( ((mlev) & 4) ? W3 : 0 ) | \
	( ((mlev) & 2) ? W2 : 0 ) | \
	( ((mlev) & 1) ? W1 : 0 )
    :	( FLAGS(player) & ~(W1|W2)    ) | \
