...
check_object(dbref player, dbref obj)
{
	...
	if(parent_loop_check(obj, LOCATION(obj))) {
	    violate(player, obj, "has a parent loop in its location");
	}
    }
...
san_fixed_log(char *format, int unparse, dbref ref1, dbref ref2)
{
	...
	log2file(LOG_SANITY, format, buf1, buf2);
    } else {
	log2file(LOG_SANITY, format, ref1, ref2);
    }
}
...
create_lostandfound(dbref *player, dbref *room)
{
	...
	*player = new_object();
	...
	add_player(*player);
	log2file(LOG_SANITY, "Using %s (with password %s) to resolve "
...
fix_player(dbref obj)
{
	...
	SanFixed2(obj, RootRoom, "Setting the home on %s to %s");
	DBFETCH(obj)->sp.player.home = RootRoom;
	DBDIRTY(obj);
    }
}
...
find_misplaced_objects(void)
{
	...
	if (TYPEOF(loop) != TYPE_GARBAGE) {
		...
		if (TYPEOF(loop) == TYPE_PLAYER ||
		    ...
		    LOCATION(loop) = EnvRoomX;
		} else if (TYPEOF(loop) == TYPE_EXIT
		) {
		    if (valid_obj(LOCATION(loop))) {
			dbref loop1 = LOCATION(loop);
			if (EXITS(loop1) == loop) {
			    EXITS(loop1) = DBFETCH(loop)->next;
			    DBDIRTY(loop1);
			} else for (loop1 = EXITS(loop1);
			       loop1 != NOTHING;
			       loop1 = DBFETCH(loop1)->next) {
			    if (DBFETCH(loop1)->next == loop) {
				DBFETCH(loop1)->next = DBFETCH(loop)->next;
...
sanfix(dbref player)
{
	...
	SanFixed(RootRoom, "Reset invalid player_start to %s");
	tp_player_start = RootRoom;
    }
	...
	if (!sanity_violated) {
	    ...
	    " @sanity.  For details of repairs, check " LOG_SANITY ".", 1);
	} else {
	...
	fprintf(stderr, "For details of repairs made, check " LOG_SANITY ".\n");
    }
    if (sanity_violated) {
	log2file(LOG_SANITY, "WARNING: The database is still corrupted, please repair by hand");
    }
}
...
void sanechange(dbref player, const char *command)
{
    ...
    if (player > NOTHING) {
	if (!Boy(player)) {
	    notify(player, "Only " NAMEMAN " may alter the basic structure of the universe!");
	    return;
	}
